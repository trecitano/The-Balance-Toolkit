...
const VCopXGraph: React.FC<VCopXGraphProps> = ({ data, containerHeight }) => {
  ...
  useEffect(() => {
    ...
    const baseFontSize = Math.max(
      8,
      Math.min(14, Math.floor((containerHeight ?? size.height) * 0.07)),
    );
    const dynamicGraphCircleDiameter = Math.max(
      4,
      Math.min(10, Math.floor((containerHeight ?? size.height) * 0.04)),
    );
    const padding = {
  ...
  return (
    <div ref={containerRef} style={{ width: "100%", height: "100%" }}>
      <svg
        ...
        className="vcopx-graph-svg"
      />
    </div>
  );
};

export default VCopXGraph;
