...
fn lsl_stream_loop_raw(mut rx: Receiver<BalanceBoardCalibratedReading>,
    ...
    // Create raw data stream (timestamp + 4 sensor readings)
    let raw_info = lsl::StreamInfo::new(
        &stream_name,
        "BalanceBoard_Raw",
        ...
        &source_id)?;
    let raw_outlet = lsl::StreamOutlet::new(&raw_info, 0, 360)?;
    ...
    while let Some(data) = rx.blocking_recv() {
        ...
        raw_outlet.push_sample(&byte_array)?;
    }
