...
#[tokio::main]
async fn main() {
    let all_bluetooth_view = wii_pin_generator::get_all_bluetooth_adapters_info().await.unwrap();
    ...
    let nintendo_board_opt = wii_pin_generator::find_nintendo_balance_board(&bluetooth_view);
    if let Some(nintendo_board) = nintendo_board_opt {
        ...
        wii_pin_generator::scan_and_pair_nintendo().await.unwrap()
    }
